defmodule UnitFun.Units.CompositeUnit do
  ...
  def multiply_unit(
        %__MODULE__{numerators: numerators} = initial,
        %{type: :primitive_unit} = unit
      ) do
  ...
  def divide_unit(
        %__MODULE__{denominators: denominators} = initial,
        %{type: :primitive_unit} = unit
      ) do
  ...
  for fun <- [:multiply_unit, :divide_unit] do
    ...
    def unquote(fun)(first_module, second_module)
        when is_atom(first_module) and is_atom(second_module) do
      first_module.unit
      |> new_with_numerator
